...
export default {
    ...
    methods: {
        handlePhotoUpload(event) {
            ...
            this.form.photo = file; // 사진 파일을 form 데이터에 저장
        },
        addEducation() {
        ...
        filterTags() {
            ...
            if (searchText) {
                // 태그 내의 모든 위치에서 부분 문자열이 일치하는지 확인
                this.filteredTags = this.predefinedTags.filter((tag) => tag.toLowerCase().includes(searchText));
            } else {
                this.filteredTags = [];
            }
        },
        ...
        async submitForm() {
            try {
                ...
                const response = await this.$axios.post(
                    `http://localhost:8080/query/memberspr/insert/${this.user.member_id}`,
                    {
                        ...this.form,
