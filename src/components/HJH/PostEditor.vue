<template>
  ...
  <!-- <div v-if="user" class="post-editor"> -->
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      // Apply custom styling to the editor after initialization
      const editorElement = quillEditor.value.querySelector('.ql-editor');
      if (editorElement) {
        editorElement.style.minHeight = '200px';  // Increase minimum height
        editorElement.style.maxHeight = '280px';  // Increase maximum height
        editorElement.style.overflowY = 'auto';   // Enable scrolling when content exceeds max height
      }
    });
...
.ql-editor {
  ...
  max-height: 500px !important; /* 원하는 최대 높이로 설정하세요 */
  border: 1px solid #ccc;
  border-radius: 4px;
  padding: 8px;
  width: 100%;
